...
public class PingModule implements HudModule {
    ...
    /**
     * Reads the actual measured round-trip time from the network connection.
     * ClientConnection#getAverageTransitTime() is updated every time a
     * keep-alive packet is exchanged (~1 s intervals), giving a live value
     * instead of the stale tab-list latency from PlayerListEntry#getLatency().
     */
    private static int getPing(MinecraftClient client) {
        ClientPlayNetworkHandler handler = client.getNetworkHandler();
        if (handler == null) return 0;
        // getAverageTransitTime() returns milliseconds as a long
        return (int) handler.getConnection().getAverageTransitTime();
    }
